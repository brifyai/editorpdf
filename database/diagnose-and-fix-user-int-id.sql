-- =====================================================
-- DIAGNÓSTICO Y REPARACIÓN DE user_int_id
-- =====================================================
...
-- =====================================================
-- VERIFICAR Y REPARAR TABLAS QUE DEPENDEN DE user_int_id
-- =====================================================

SELECT '=== VERIFICANDO TABLAS RELACIONADAS ===' as info;

-- Lista de tablas que deberían tener user_int_id
WITH tablas_a_verificar AS (
    SELECT 'documents' as tabla UNION
    SELECT 'document_analyses' UNION
    SELECT 'batch_jobs' UNION
    SELECT 'batch_job_files' UNION
    SELECT 'user_profiles' UNION
    SELECT 'user_preferences' UNION
    SELECT 'user_usage_stats' UNION
    SELECT 'ai_model_metrics'
)
SELECT 
    tv.tabla,
    EXISTS(
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = tv.tabla AND column_name = 'user_int_id'
    ) as tiene_user_int_id
FROM tablas_a_verificar tv;
...
CREATE POLICY IF NOT EXISTS "Users can view their own batch jobs" ON batch_jobs
    FOR SELECT USING (user_int_id = current_setting('app.current_user_id', true)::INTEGER);

CREATE POLICY IF NOT EXISTS "Users can create their own batch jobs" ON batch_jobs
    FOR INSERT WITH CHECK (user_int_id = current_setting('app.current_user_id', true)::INTEGER);

CREATE POLICY IF NOT EXISTS "Users can update their own batch jobs" ON batch_jobs
    FOR UPDATE USING (user_int_id = current_setting('app.current_user_id', true)::INTEGER);

CREATE POLICY IF NOT EXISTS "Users can delete their own batch jobs" ON batch_jobs
    FOR DELETE USING (user_int_id = current_setting('app.current_user_id', true)::INTEGER);

-- Políticas similares para las otras tablas...
CREATE POLICY IF NOT EXISTS "Users can manage their own profiles" ON user_profiles
...
CREATE POLICY IF NOT EXISTS "Users can view their own usage stats" ON user_usage_stats
    FOR SELECT USING (user_int_id = current_setting('app.current_user_id', true)::INTEGER);

CREATE POLICY IF NOT EXISTS "Users can insert their own usage stats" ON user_usage_stats
    FOR INSERT WITH CHECK (user_int_id = current_setting('app.current_user_id', true)::INTEGER);
...
SELECT '✅ DIAGNÓSTICO Y REPARACIÓN COMPLETADOS' as status;